...
export function CsgHistoryCard({ transactions, stats }: { transactions: DashboardTx[]; stats: DashboardHeroStats }) {
  ...
  const globalTotals = summarizeCsgHistory(annual);
  ...
  return (
    <section className={dashboardInsightCard} aria-label="Historique CSG par année">
      ...
      <ul className="mt-4 grid grid-cols-2 gap-2 text-xs">
        {[
          { label: "CSG 9,7 %", value: globalTotals.csg97Eur, dot: "bg-emerald-500 dark:bg-emerald-400" },
          { label: "CSG 17,2 %", value: globalTotals.csg172Eur, dot: "bg-sky-500 dark:bg-sky-400" }
        ].map((item) => (
      ...
      <details className="mt-3 border-t border-ink-200/50 pt-2 dark:border-white/[0.08]">
        <summary className="min-h-11 cursor-pointer content-center rounded-md text-xs font-medium text-ink-600 transition-colors hover:text-ink-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 dark:text-white/60 dark:hover:text-white">
          Détail par année
        </summary>
        <div className="mt-2">
