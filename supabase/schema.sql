...
drop policy if exists "billable_commute_days_own" on public.billable_commute_days;
create policy "billable_commute_days_own"
on public.billable_commute_days for all
using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "billable_mileage_adjustments_own" on public.billable_mileage_adjustments;
